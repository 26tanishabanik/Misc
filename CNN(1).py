...
class myCallback(tf.keras.callbacks.Callback):
  def on_epoch_end(self,epoch,logs= {}):
    if(logs.get('acc')>0.98 and logs.get('val_acc')>0.90):
...
model = tf.keras.models.Sequential([
    ...
    tf.keras.layers.Dense(4096, activation = 'relu', input_shape=(4096, 1)),
...
hist_json_file = 'CNNhistory4.json'
with open(hist_json_file, mode='w') as f:
    hist_df.to_json(f)
model.save('pic4_NN.h5')
